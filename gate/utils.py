...
# https://github.com/pytorch/fairseq/blob/129d8594ccdc6644be84dc249e16489e049f4bfd/fairseq/utils.py#L265
def buffered_arange(min, max, dtype, device):
    if not (hasattr(buffered_arange, "buf") and buffered_arange.buf.device == device):
        buffered_arange.buf = torch.arange(max, device=device, dtype=dtype)
    if max > buffered_arange.buf.numel():
        buffered_arange.buf.resize_(max)
        torch.arange(max, out=buffered_arange.buf)
    return buffered_arange.buf[min:max]


def _make_ix_like(input, dim=0):
    d = input.size(dim)
    # rho = torch.arange(1, d + 1, device=input.device, dtype=input.dtype)
    # Buffered arange to speed up computation
    rho = buffered_arange(min=1, max=d + 1, device=input.device, dtype=input.dtype)
    view = [1] * input.dim()
    view[0] = -1
...
class Entmax15Function(Function):
    ...
    def forward(ctx, input, dim=-1):
        ...
        max_val = torch.index_select(Xsrt, dim, torch.tensor([0], device=input.device))
        input = input - max_val  # same numerical stability trick as for softmax
        input = input / 2  # divide by 2 to solve actual Entmax
        Xsrt = Xsrt - max_val  # same numerical stability trick as for softmax
        Xsrt = Xsrt / 2  # divide by 2 to solve actual Entmax

        tau_star, _ = Entmax15Function._threshold_and_support(Xsrt, dim)
    ...
    @staticmethod
    def _threshold_and_support(Xsrt, dim=-1):
